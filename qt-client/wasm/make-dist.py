...
can keep using the previous DailyHexPuzzle.js/.wasm for ten minutes after a
...
served stale, and it always points at assets that match it.
    ...
    python3 qt-client/wasm/make-dist.py <build-dir> <out-dir>

coi-serviceworker.js keeps its plain name on purpose: it is registered by URL
and a stable one keeps a single service worker registration across deploys.
"""
...
def main(build_dir: str, out_dir: str) -> int:
    ...
    missing = [f for f in (f"{APP}.js", f"{APP}.wasm", "qtloader.js",
                           "index.html", "coi-serviceworker.js")
    ...
    # ── service worker: stable name, see module docstring ──────────────────
    shutil.copyfile(build / "coi-serviceworker.js", out / "coi-serviceworker.js")
